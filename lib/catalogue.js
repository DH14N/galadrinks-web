...
import { categories, getCategory, getFormat } from "./categories";

// Re-export so pages can import these straight from the catalogue,
// while this module can also use `categories` in its own helpers.
export { categories, getCategory, getFormat };
...
// A recognisable product to illustrate each category tile on the
// homepage. Preferred names are tried first; otherwise the first
// product in the range that has a photo is used.
const CATEGORY_IMAGE_KEYWORDS = {
  "lagers-craft-beers": ["peroni nastro", "stella artois", "birra moretti"],
  "ales-bitters-stouts": ["guinness draught", "guinness original"],
  cider: ["kopparberg strawberry", "strongbow dark fruit", "strongbow original"],
  spirits: ["jack daniel's old no", "smirnoff red", "gordon's london"],
  "liqueurs-speciality": ["baileys original", "jagermeister", "disaronno"],
  rtds: ["wkd blue", "smirnoff ice", "au vodka"],
  wine: ["barefoot pinot", "casillero", "yellow tail"],
  "champagne-prosecco": ["veuve clicquot", "moet et chandon", "freixenet prosecco"],
  kegs: ["heineken", "peroni", "guinness"],
  miniatures: ["miniature"],
  "gift-sets": ["gift set", "gift pack"],
  "soft-drinks": ["coca-cola original taste 24", "coca cola 24", "pepsi cola 24"],
  "energy-drinks": ["red bull energy", "monster energy"],
  snacks: ["doritos", "walkers ready salted", "mccoy"],
  "hot-beverages": ["nescafe", "yorkshire tea", "pg tips"],
  "no-low-alcohol": ["heineken alcohol free lager bottles", "san miguel 0", "corona cero alcohol free beer 24"],
  "bar-supplies": ["pint glass", "glass"],
};

// Formats/variants we don't want fronting a range tile
const IMG_EXCLUDE = /postmix|bag in box|syrup|multipack|variety|mixed (case|pack)/i;

export function getCategoryImages() {
  const out = {};
  for (const c of categories) {
    let inCat = getProductsInCategory(c.slug).filter(
      (p) => p.image_url && !IMG_EXCLUDE.test(p.name)
    );
    // For normal ranges, don't front the tile with an alcohol-free variant
    if (c.slug !== "no-low-alcohol") {
      const full = inCat.filter((p) => !p.nolow);
      if (full.length) inCat = full;
    }

    const kws = CATEGORY_IMAGE_KEYWORDS[c.slug] || [];
    let img = null;
    for (const kw of kws) {
      const p = inCat.find((x) => x.name.toLowerCase().includes(kw));
      if (p) {
        img = p.image_url;
        break;
      }
    }
    if (!img && inCat.length) img = inCat[0].image_url;
    out[c.slug] = img;
  }
  return out;
}

...
// What's currently set up, for the check on the admin dashboard.
// Never returns the password itself — only whether one is present.
export function emailSettings() {
  const host = process.env.SMTP_HOST || "";
  const user = process.env.SMTP_USER || "";
  const hasPassword = Boolean(process.env.SMTP_PASSWORD);
  const smtpReady = Boolean(host && user && hasPassword);

  return {
    mode: smtpReady ? "smtp" : process.env.RESEND_API_KEY ? "resend" : "none",
    host,
    ...
    missing: [
      ...
      !process.env.ORDERS_EMAIL && "ORDERS_EMAIL",
    ].filter(Boolean),
  };
}

// Opens a connection to the mailbox and signs in, without sending anything.
export async function verifyEmail() {
  const mailer = smtpTransport();
  if (!mailer) return { ok: false, error: "SMTP isn’t set up yet." };
  try {
    await mailer.verify();
    return { ok: true };
  } catch (err) {
...
// Sent by the "send test email" button in the admin area
export function testEmail({ sentBy, when }) {
  return shell(
    "Test message",
    `<p style="margin:0 0 6px;font-size:17px;font-weight:bold">Email is working.</p>
     <p style="margin:0;font-size:14px;color:#5a6474;line-height:1.6">
       This was sent from the Gala Drinks website to check the mailbox settings.
       Order confirmations and website enquiries will go out the same way.
     </p>
     <p style="margin:18px 0 0;padding:12px 14px;background:#f3efe7;border-radius:8px;font-size:13px;color:#5a6474">
       Requested by ${esc(sentBy)} at ${esc(when)}.
     </p>`
  );
}

...
// Same as above, but also requires the account to be an admin.
// Every admin API uses this — being an admin is checked here on the
// server, never taken from anything the browser says.
export async function requireAdmin(request) {
  const result = await getCustomerFromRequest(request);
  if (result.error) return result;
  if (!result.customer.is_admin) return { error: "not-admin" };
  return result;
}
...
export function authErrorResponse(error) {
  const map = {
    ...
    "not-admin": [403, "You don’t have access to this area."],
  };
